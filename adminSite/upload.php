...
$target_dir = "../displaySite/img/large_ad/";

$big_image = $target_dir . basename($_FILES["fileToUpload"]["name"]);

// if everything is ok, try to upload file
if (imageCheck($big_image, "fileToUpload") == True) {
...
function imageCheck($target_file, $file_name) {
    ...
    if(isset($_POST["submit"])) {
        $check = getimagesize($_FILES[$file_name]["tmp_name"]); //To-Do: Update fileToUpload to a variable?
        if($check == false) {
